...
async def run_shopping_agent(
    ...
    2. Generate a high-level plan using the CoT agent.
    3. Run the shopping agent with the combined input and plan.
    4. If the output message exists, normalize it via the parser agent.
    
    Returns:
        Tuple[ShoppingResponse, dict]: 
        - ShoppingResponse object with structured fields (message, base_random_keys, member_random_keys).
        - Dictionary representation of the output with the normalized message applied.
          The 'message' field is guaranteed to be normalized (cleaned and human-friendly),
          while the random keys remain as generated by the shopping agent.

    If an error occurs, returns:
        - None for the ShoppingResponse
        - Dictionary with an error message in 'message' and None for key lists.
    """
    try:
        ...
        # Step 4: normalize message if exists
