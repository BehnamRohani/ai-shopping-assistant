...
1. Identify user intent clearly:
   ...
   - Else if the user asks for **seller/shop-related info** (availability, price (قیمت), members (عضو), shops (فروشگاه)) → resolve via SQL.  
   ...
   - Only if the product request is **vague or incomplete** or **initating a conversation to find a suitable product and seller** → (interactive narrowing until a member_random_key can be determined)?
      • Clarification: Treat vague or generic product requests (e.g., "یه لپ تاپ خوب می‌خوام", "دنبال یه فروشنده خوب میگردم، میتونی که کمکم کنی؟") as conversation initiation.
